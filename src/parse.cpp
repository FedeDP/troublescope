...
static inline sinsp_param get_syscall_evt_param(void *evt, uint32_t num_param) {
  ...
  auto len =
      (T)((uint8_t *)evt + sizeof(falcosecurity::_internal::ss_plugin_event));
  for (uint32_t j = 0; j < num_param; j++) {
    // sum lengths of the previous params.
    dataoffset += len[j];
  }
  return {
      .param_len = len[num_param],
      .param_pointer =
          ((uint8_t *)&len[((falcosecurity::_internal::ss_plugin_event *)evt)
                               ->nparams]) +
          dataoffset};
}
...
bool my_plugin::parse_new_process_event(
  ...
  try {
    ...
    SPDLOG_ERROR("cannot attach fuseFS entry to new process event for the "
                 "thread id '{}': {}",
                 thread_id, e.what());
    return false;
  }
