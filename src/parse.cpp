...
bool my_plugin::parse_diff_async_event(const falcosecurity::parse_event_input &in) {
...
bool my_plugin::parse_async_event(const falcosecurity::parse_event_input &in) {
	...
	if(!is_root && !is_pid && !is_entry && !is_diff) {
	...
	if(is_diff) {
		// We are not interested in parsing the diff event.
		return parse_diff_async_event(in);
	}
